...
class AdminController extends Controller {
    ...
    public function dashboard() {
        $pageTitle = 'Dashboard';
        $orderQuery        = Order::whereNotIn('status', [Status::ORDER_UNPAID, Status::ORDER_REJECT]);
        ...
        $widget['total_order_count']       = $orderQuery->count();
        $widget['total_order_amount']      = $orderQuery->sum('amount');


        $payment['total_payment_amount']        = Deposit::where('order_id', '!=', 0)->successful()->sum('amount');
        $payment['total_payment_pending']       = Deposit::where('order_id', '!=', 0)->pending()->sum('amount');
        $payment['total_payment_rejected']      = Deposit::where('order_id', '!=', 0)->rejected()->sum('amount');
        $payment['total_payment_charge']        = Deposit::where('order_id', '!=', 0)->successful()->sum('charge');

        $deposit['total_deposit_amount']        = Deposit::where('order_id', 0)->successful()->sum('amount');
        $deposit['total_deposit_pending']       = Deposit::where('order_id', 0)->pending()->count();
        $deposit['total_deposit_rejected']      = Deposit::where('order_id', 0)->rejected()->count();
        $deposit['total_deposit_charge']        = Deposit::where('order_id', 0)->successful()->sum('charge');
        ...
        $miningTrack['total_tracks']            = Order::approved()->count();
        $miningTrack['active_tracks']           = Order::running()->count();
        $miningTrack['completed_tracks']        = Order::completed()->count();

        $transactions = Transaction::leftJoin('miners', 'transactions.currency', '=', 'miners.currency_code')
            ->selectRaw("
                SUM(CASE WHEN transactions.remark = 'return_amount' THEN transactions.amount * miners.rate ELSE 0 END) AS total_return_amount,
                SUM(CASE WHEN transactions.remark = 'maintenance_cost' THEN transactions.amount * miners.rate ELSE 0 END) AS total_maintenance_cost
            ")
            ->first();

        $miningTrack['total_returned']          = $transactions->total_return_amount;
        $miningTrack['total_maintenance_cost']  = $transactions->total_maintenance_cost;


        $cryptoCurrencies = Miner::select('currency_code')->get()->pluck('currency_code')->toArray();
        $allCurrencies = [...$cryptoCurrencies, gs('cur_text')];

        // user Browsing, Country, Operating Log
        $userLoginData = UserLogin::where('created_at', '>=', Carbon::now()->subDays(30))->get(['browser', 'os', 'country']);

        $chart['user_browser_counter'] = $userLoginData->groupBy('browser')->map(fn($item, $key) => collect($item)->count());
        $chart['user_os_counter']      = $userLoginData->groupBy('os')->map(fn($item, $key) => collect($item)->count());
        $chart['user_country_counter'] = $userLoginData->groupBy('country')->map(fn($item, $key) => collect($item)->count())->sort()->reverse()->take(5);

        return view('admin.dashboard', compact('pageTitle', 'widget', 'chart', 'withdrawals', 'allCurrencies', 'cryptoCurrencies', 'deposit', 'withdrawals', 'payment', 'miningTrack'));
    }
    ...
    public function transactionReport(Request $request) {
        ...
        foreach ($dates as $date) {
            $data[] = [
                'created_on' => $date,
                'credits' => trimTrailingZeros($plusTransactions->where('created_on', $date)->first()?->amount ?? 0),
                'debits' => trimTrailingZeros($minusTransactions->where('created_on', $date)->first()?->amount ?? 0)
            ];
        }
    ...
    public function returnAmountReport(Request $request) {
        ...
        foreach ($dates as $date) {
            $data[] = [
                'created_on' => $date,
                'returnAmount' => trimTrailingZeros($transactions->where('created_on', $date)->first()?->amount ?? 0),
            ];
        }
