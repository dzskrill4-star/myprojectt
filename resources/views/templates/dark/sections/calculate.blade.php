                    ...
                    <form action="#">
                        ...
                        <div class="row gy-3 align-items-center">
                            <div class="col-lg-4 col-sm-6">
                                ...
                                <select class="custom--select bg--dark w-100" name="miner">
                                    <option value="" disabled>@lang('Select Coin')</option>
                                    @foreach ($miners as $miner)
                                        <option data-currency_code="{{ strtoupper($miner->currency_code) }}" data-plans="{{ $miner->plans }}" value="{{ $miner->id }}" @selected($loop->first)>{{ __($miner->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-4 col-sm-6">
                                <label class="text--base">@lang('Select Plan')</label>
                                <select class="custom--select revenue-calculate plans bg--dark w-100">
    ...
    <script>
        ...
        (function($) {
            $('select[name="miner"]').on('change', function() {
                ...
                var currency_code = $(this).find(':selected').data('currency_code');
                ...
                if (plans.length != 0) {
                    $.each(plans, function(key, plan) {
                        ...
                        output += `<option value="${per_day} ${currency_code}"> ${plan.title} </option>`;
                    });
