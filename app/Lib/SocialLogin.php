...
class SocialLogin {
    ...
    public function login() {
        ...
        //for 2fa
        $userData->tv = $userData->ts == Status::VERIFIED ? Status::UNVERIFIED : Status::VERIFIED;
        $userData->save();
    ...
    private function createUser($user, $provider) {
        ...
        if (isset($user->email) && $user->email) {
            $baseUsername = explode('@', strtolower($user->email))[0] ?? '';
        }

        if (!$baseUsername && isset($user->name) && $user->name) {
        ...
        if (strlen($baseUsername) < 3) {
            $baseUsername = 'user_' . strtolower(getTrx(6));
        }

        $username = $baseUsername;
        $counter  = 0;
        while (User::whereRaw('LOWER(username) = ?', [Str::lower($username)])->exists()) {
            $counter++;
            $suffix   = '_' . strtolower(getTrx(4));
            $username = substr($baseUsername, 0, 50 - strlen($suffix)) . $suffix;
            if ($counter > 20) {
                $username = 'user_' . strtolower(getTrx(10));
                break;
            }
        }
