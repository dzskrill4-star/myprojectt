...
class Order extends Model {
    ...
    public function scopeApproved($query) {
        $query->where('orders.status', Status::ORDER_APPROVED);
    }

    public function scopeUnpaid($query) {
        $query->where('orders.status', Status::ORDER_UNPAID);
    }

    public function scopePending($query) {
        $query->where('orders.status', Status::ORDER_PENDING);
    }

    public function scopeRejected($query) {
        $query->where('orders.status', Status::ORDER_REJECT);
    }

    public function scopeRunning($query) {
        $query->where('orders.status', Status::ORDER_APPROVED)->where('period_remain', '>', 0);
    }
