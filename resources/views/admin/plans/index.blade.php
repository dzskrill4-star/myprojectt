                            ...
                            <tbody class="list">
                                @forelse($plans as $plan)
                                    <tr>
                                        ...
                                        <td> {{ $plan->returnPerDay }} {{ strtoupper($plan->miner->currency_code) }} </td>
                                        ...
                                        <td>
                                            <div class="button--group">
                                                ...
                                                @else
                                                    <button type="button" class="btn btn-sm btn-outline--danger confirmationBtn" data-action="{{ route('admin.plan.status', $plan->id) }}" data-question="@lang('Are you sure to disable this plan?')">
                                                        <i class="la la-eye-slash"></i>@lang('Disable')
                                                    </button>
                                                @endif

                                                <a class="btn btn-outline--info btn-sm" href="{{ route('admin.plan.overview', $plan->id) }}">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <select class="select2 addModal form-control" data-minimum-results-for-search="-1" name="miner" disabled>
                                    <option value="">@lang('Select One')</option>
                                    @foreach ($miners as $miner)
                                        <option data-currency_code={{ $miner->currency_code }} value="{{ $miner->id }}" @selected($miner->id == old('miner'))> {{ __($miner->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-6">
                                ...
                                <select class="select2 addModal form-control" data-minimum-results-for-search="-1" name="return_type">
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <div class="input-group">
                                    <input class="form-control" name="speed" type="number" step="any" placeholder="@lang('Enter Hashrate Value')" required />
                                    <select class="input-group-text" name="speed_unit">
                                        <option value="0">@lang('H/s')</option>
                                        <option value="1">@lang('KH/s')</option>
                                        <option value="2" selected>@lang('MH/s')</option>
                                        <option value="3">@lang('GH/s')</option>
                                        <option value="4">@lang('TH/s')</option>
                                        <option value="5">@lang('PH/s')</option>
                                        <option value="6">@lang('EH/s')</option>
                                        <option value="7">@lang('ZH/s')</option>
                                        <option value="8">@lang('YH/s')</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group col-lg-4">
                                ...
                                <div class="input-group">
                                    <input class="form-control" name="period" type="number" step="any" required />
                ...
                <form id="editForm" action="" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="form-group col-lg-6">
                                <label>@lang('Currency')</label>
                                <select class="select2 form-control" data-minimum-results-for-search="-1" name="miner">
                                    <option value="">@lang('Select One')</option>
                                    @foreach ($miners as $miner)
                                        <option data-currency_code={{ $miner->currency_code }} value="{{ $miner->id }}" @selected(old('miner') == $miner->id)> {{ __($miner->name) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-6">
                                <label>@lang('Return Amount Type')</label>
                                <select class="select2 form-control" data-minimum-results-for-search="-1" name="return_type">
                                    <option value="1" @selected(old('return_type') == 1)>@lang('Fixed')</option>
                                    <option value="2" @selected(old('return_type') == 2)>@lang('Random')</option>
                                </select>
                            </div>
...
@push('breadcrumb-plugins')
    @if (request()->routeIs('admin.plan.index'))
        <x-search-form placeholder="Title / Currency" />
    @else
        <a class="btn btn-sm btn-outline--dark" href="{{ route('admin.currency.index') }}"><i class="las la-undo"></i>@lang('Back')</a>
    @endif
...
@push('script')
    <script>
        ...
        (function($) {
            ...
            $.each($('.select2.addModal'), function() {
            ...
            $('.addBtn').on('click', function() {
                ...
                modal.find('select[name=miner]').attr('disabled', false);
            ...
            $('.editBtn').on('click', function() {
                ...
                modal.find('.rpd_cur_sym').text(plan.miner.currency_code);
                modal.find('select[name=miner]').val(plan.miner_id).attr('disabled', true);
                ...
                var coinCode = modal.find('select[name=miner]').find(':selected').attr('data-currency_code');
            ...
            $(document).on('change', 'select[name=miner]', function() {
                var coinCode = $(this).find(':selected').attr('data-currency_code');
