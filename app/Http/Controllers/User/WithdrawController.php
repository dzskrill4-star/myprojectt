...
class WithdrawController extends Controller
{
    ...
    public function withdrawStore(Request $request)
    {
        ...
        // Check if Baridi Mob is selected and user has permission
        if (strtolower(str_replace(' ', '', $method->name)) === 'baridimob' && !$user->baridi_withdraw) {
            $notify[] = ['error', 'You do not have permission to use Baridi Mob withdrawal method'];
        ...
        if ($request->amount > $user->profit_wallet) {
            $notify[] = ['error', 'Insufficient profit balance for withdrawal'];
            return back()->withNotify($notify)->withInput($request->all());
        }
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        $formData = isset($method->form->form_data) ? $method->form->form_data : [];
        ...
        notify($user, 'WITHDRAW_REQUEST', [
            ...
            'trx' => $withdraw->trx,
            'post_balance' => showAmount($user->profit_wallet, currencyFormat: false),
        ]);
