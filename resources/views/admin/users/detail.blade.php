        ...
        <div class="col-12">
            <div class="row gy-4">
                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.report.transaction', $user->id) }}?wallet_type={{Status::DEPOSIT_WALLET}}" title="Deposit Wallet Balance" icon="las la-money-bill-wave-alt" value="{{ showAmount($user->balance) }}" bg="success" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.report.transaction', $user->id) }}?wallet_type={{Status::PROFIT_WALLET}}" title="Earning Wallet Balance" icon="las la-money-bill-wave-alt" value="{{ showAmount($user->balance) }}" bg="primary" type="2" />
                </div>


                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.report.transaction', $user->id) }}" title="Transactions" icon="las la-exchange-alt" value="{{ $totalTransaction }}" bg="17" type="2" />
                </div>


                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.deposit.successful', $user->id) }}" title="Total Deposited" icon="las la-wallet" value="{{ showAmount($totalDeposit) }}" bg="green" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.withdraw.data.all', $user->id) }}" title="Total Withdrawn" icon="las la-wallet" value="{{ showAmount($totalWithdrawal) }}" bg="5" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.deposit.pending', $user->id) }}" title="Pending Deposits" icon="las la-file-invoice-dollar" value="{{ $pendingDeposits }}" bg="warning" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.withdraw.data.pending', $user->id) }}" title="Pending Withdrawals" icon="las la-wallet" value="{{ $pendingWithdrawals }}" bg="6" type="2" />
                </div>

                <div class="col-xxl-3 col-sm-6">
                    <x-widget style="6" link="{{ route('admin.ticket.pending', $user->id) }}" title="Pending Support Tickets" icon="la la-ticket" value="{{ $pendingTickets }}" bg="14" type="2" />
                </div>
            </div>
    ...
    <script>
        (function($) {
            ...
            $('.bal-btn').on('click', function() {
                ...
                if (act == 'add') {
                    $('#addSubModal .modal-title').text('Add Balance to Deposit Wallet');
                } else {
                    $('#addSubModal .modal-title').text('Subtract Balance from Deposit Wallet');
                }
            });
