...
class FrontendController extends Controller {
    ...
    public function frontendSections($key, $parentId = null, $level = 0) {
        ...
        if ($parentId) {
            $parentElement = Frontend::findOrFail($parentId);
            $grandParentId = $parentElement->parent_id;
        }

        $elements = Frontend::where('parent_id', $parentId)->where('data_keys', $key . '.element')->where('tempname', activeTemplateName())->orderBy('sort_order', 'asc')->get();

        $elementKeys = $section->element ?? null;

        for ($i = 0; $i < $level; $i++) {
            $elementKeys = $elementKeys->element;
        }
        ...
        return view('admin.frontend.section', compact('section', 'content', 'elements', 'key', 'pageTitle', 'elementKeys', 'level', 'parentId', 'grandParentId'));
    }
    ...
    public function frontendElement(Request $request, $key, $id = null) {
        $section = isset(getPageSections()->$key) ? getPageSections()->$key : null;
        if (!$section) {
            return abort(404);
        }
        ...
        return view('admin.frontend.element', compact('section', 'key', 'pageTitle', 'data', 'elementKeys'));
    }
    ...
    public function updateElementsOrder(Request $request, $key) {
        $request->validate([
            'ids' => 'required|array'
        ]);
        ...
        foreach ($ids as $index => $id) {
            if (isset($elements[$id])) {
                $elements[$id]->update(['sort_order' => $index + 1]);
            }
        }
