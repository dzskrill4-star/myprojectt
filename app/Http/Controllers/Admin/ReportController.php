...
class ReportController extends Controller {
    public function transaction(Request $request, $userId = null) {
        ...
        $currencies = Transaction::distinct('currency')->orderBy('currency')->get('currency');

        $transactions = Transaction::searchable(['trx', 'user:username'])->filter(['trx_type', 'remark',  'currency'])->dateFilter()->orderBy('transactions.id', 'desc')->with('user');
        if ($userId) {
        ...
        if ($request->plan_id) {
            $transactions = $transactions->leftJoin('orders', 'transactions.order_id', 'orders.id')->where('orders.plan_id', $request->plan_id);
        }
    ...
    public function referralBonusLog() {
        ...
        $logs       = ReferralLog::with('user', 'referee')->orderBy('id', 'desc')->paginate(getPaginate());

        return view('admin.reports.referral_bonus_log', compact('pageTitle', 'logs'));
    }
}
