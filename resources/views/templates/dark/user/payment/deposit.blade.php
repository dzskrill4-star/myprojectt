    ...
    <div class="card custom--card">
        <div class="card-body">
            <form action="{{ route('user.deposit.insert') }}" method="post" class="deposit-form">
                ...
                <input name="order" type="hidden" value="{{ $order->id ?? 0 }}">
                                    ...
                                    <label for="{{ titleToKey($data->name) }}" class="payment-item gateway-option">
                                        <div class="payment-item__info">
                                            ...
                                            <span class="payment-item__name">
                                                {{ $isBaridi ? 'Deposit DZD (Baridi Mob)' : __($data->name) }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="payment-item__thumb">
                                            @php
                                                $customImage = null;
                                                if ($isUSDT) {
                                                    $customImage = asset('assets/images/payments/usdt.png');
                                                }
                                                if ($isBaridi) {
                                                    $customImage = asset('assets/images/payments/baridimob.png');
                                                }
                                            @endphp

                                            <img class="payment-item__thumb-img"
                                                src="{{ $customImage ?: getImage(getFilePath('gateway') . '/' . $data->method->image) }}"
                                                alt="{{ $isBaridi ? 'Deposit DZD (Baridi Mob)' : $data->name }}">
                                        </div>
                                ...
                                <div class="deposit-info">
                                    ...
                                    <div class="deposit-info__input">
                                        <div class="deposit-info__input-group input-group">
                                            ...
                                            <input class="form-control form--control amount" name="amount" type="text" value="{{ old('amount', isset($order) ? getAmount($order->amount) : '') }}" @if (isset($order)) readonly @endif autocomplete="off">
                                ...
                                <div class="deposit-info">
                                    <div class="deposit-info__title">
                                        <p class="text has-icon">@lang('Processing Charge')
                                            <span data-bs-toggle="tooltip" title="@lang('Processing charge for payment gateways')" class="processing-fee-info"><i class="las la-info-circle"></i> </span>
                                        </p>
                                    </div>
                                    <div class="deposit-info__input">
