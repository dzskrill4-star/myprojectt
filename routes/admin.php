...
Route::namespace('Auth')->group(function () {
    Route::middleware('admin.guest')->group(function () {
        Route::controller('LoginController')->group(function () {
            ...
            })->middleware('admin')->withoutMiddleware('admin.guest')->name('logout');
            Route::post('logout', 'logout')->middleware('admin')->withoutMiddleware('admin.guest');
        });
...
Route::middleware('admin')->group(function () {
    Route::controller('AdminController')->group(function () {
        ...
        Route::get('transaction-report', 'transactionReport')->name('chart.transaction');
        Route::get('return-amount-report', 'returnAmountReport')->name('chart.return.amount');
        ...
        Route::get('notification/read/{id}', function (Request $request) {
            return view('shared.csrf_handoff', [
                'action' => $request->url(),
                'returnUrl' => url()->previous(),
            ]);
        })->name('notification.read');
        Route::get('notifications/read-all', function (Request $request) {
            return view('shared.csrf_handoff', [
                'action' => $request->url(),
                'returnUrl' => url()->previous(),
            ]);
        })->name('notifications.read.all');
        Route::post('notifications/read-all', 'readAllNotification');
        ...
        Route::post('notification/read/{id}', 'notificationRead');
    ...
    // Miner
    Route::controller('CryptocurrencyController')->prefix('currencies')->name('currency.')->group(function () {
        Route::get('list', 'index')->name('index');
        Route::post('store/{id?}', 'store')->name('store');
        Route::get('analytics/{id}', 'overview')->name('overview');
        Route::get('orders', 'orderAnalytics')->name('order.analytics');
        Route::get('returns', 'returnAnalytics')->name('return.analytics');
        Route::get('orders-by-plan', 'ordersByPlan')->name('orders.by.plan');
    });

    // Plans
    Route::controller('PlanController')->prefix('mining-plans')->name('plan.')->group(function () {
    ...
    Route::controller('MiningTracksController')->group(function () {
        Route::get('mining-tracks/all', 'allTracks')->name('mining.tracks.all');
        Route::get('mining-tracks/active', 'activeTracks')->name('mining.tracks.active');
        Route::get('mining-tracks/completed', 'completedTracks')->name('mining.tracks.completed');
    });

    Route::controller('OrderController')->prefix('orders')->name('order.')->group(function () {
        Route::get('/', 'index')->name('index');
        Route::get('running/mining-tracks', 'runningOrders')->name('running');
        Route::get('pending/mining-tracks', 'pendingOrders')->name('pending');
        Route::get('rejected/mining-tracks', 'rejectedOrders')->name('rejected');
    });

    //Badge
    Route::controller('BadgeController')->group(function () {
        Route::get('mining-badge', 'index')->name('badge.index');
        Route::post('mining-badge/save/{id?}', 'save')->name('badge.save');
        Route::post('status/{id}', 'status')->name('badge.status');
    });
    ...
    // Deposit
    Route::controller('ManageDepositController')->name('deposit.')->prefix('deposit')->group(function () {
        Route::get('/', 'deposit')->name('list');
        Route::get('pending/{userId?}', 'pending')->name('pending');
        Route::get('rejected/{userId?}', 'rejected')->name('rejected');
        Route::get('approved/{userId?}', 'approved')->name('approved');
        Route::get('successful/{userId?}', 'successful')->name('successful');
        Route::get('initiated/{userId?}', 'initiated')->name('initiated');
    ...
    Route::controller('SupportTicketController')->prefix('ticket')->name('ticket.')->group(function () {
        ...
        Route::get('pending/{userId?}', 'pendingTicket')->name('pending');
    ...
    Route::controller('ReferralController')->name('setting.')->group(function () {
    ...
    Route::controller('SystemController')->name('system.')->prefix('system')->group(function () {
        ...
        Route::post('optimize-clear', 'optimizeClear');
    ...
    Route::name('frontend.')->prefix('frontend')->group(function () {
        ...
        Route::controller('FrontendController')->group(function () {
            ...
            Route::get('frontend-sections/{key?}/{parentId?}/{level?}', 'frontendSections')->name('sections');
            Route::post('frontend-content/{key}', 'frontendContent')->name('sections.content');
            Route::get('frontend-element/{key}/{id?}', 'frontendElement')->name('sections.element');
            Route::get('frontend-element-items/{key}/{id?}', 'frontendItems')->name('sections.element.items');
            ...
            Route::post('update-elements-order/{key}', 'updateElementsOrder')->name('sections.element.sort');
        });
