(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".dashboard-menu__item.has-submenu > a").on("click", function () {
      $.each(
        $("ul.sub-menu").not($(this).parent().children("ul.sub-menu")),
        function (i, element) {
          if ($(element).css("display") === "block") {
            $(element).slideToggle(300);
            $(element)
              .parents("li.has-submenu")
              .find(".dashboard-menu__link-arrow")
              .removeClass("show");
          }
        }
      );
    ...
    $(".dashboard-sidebar__close-icon, .sidebar-overlay").on(
      "click",
      function () {
  ...
  $.each($("input, select, textarea"), function (i, element) {
    if (element.hasAttribute("required")) {
      if ($(element).parent().hasClass('input-group')) {
        $(element).parent().siblings('label').first().addClass("required")
      } else {
        $(element).siblings("label").first().addClass("required");
      }
    }
  });
